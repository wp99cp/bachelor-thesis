...
def next_image():
    ...
    if queued_refs.qsize() < 5:
        # run the datahandler in a separate thread
        thread = threading.Thread(target=load_next)
        thread.start()

    if queued_refs.empty() and thread is not None:

        # spin until the datahandler has finished
        while queued_refs.empty():
            pass

    assert not queued_refs.empty(), "The queue is empty, but the datahandler is not running"
