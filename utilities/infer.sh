...
CONFIG_FILE_PATH="$BASE_DIR/infer_config.yml"
...
if [[ -z "${RUNS_ON_EULER}" ]]; then
  python3 "$BASE_DIR/src/models/unet/infer.py"
else
  ...
  echo "RUN python '$BASE_DIR/models/unet/infer.py'"

  python3 -u "$BASE_DIR/src/models/unet/infer.py" \
    1>"$LOG_DIR/python_infer_model.log" \
    2>"$LOG_DIR/python_infer_model.error" &
